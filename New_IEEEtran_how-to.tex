...
To address these challenges, this article proposes a novel multimodal framework for few-shot remote sensing scene classification. Targeting the semantic deviation common to large language models in the remote sensing domain, we introduce the Visual Grounded Semantic Refinement (VGSR) module. This module directly anchors to visual evidence in remote sensing images to suppress semantic hallucinations during text generation, providing reliable prior knowledge for the entire network. Existing methods often perform late fusion exclusively at the classifier level, where background noise and target features remain heavily entangled. To overcome this structural limitation, we embed the Dual Cross-Modal Attention (DCMA) module deep within the intermediate stages of the visual backbone. Using the VGSR-generated text descriptions as a semantic prior, DCMA executes cross-modal interactions across spatial and channel dimensions. This actively suppresses complex background clutter during feature extraction and enhances discriminative target features, avoiding the loss of spatial layout information typically caused by global pooling operations.

Traditional mean aggregation strategies are highly sensitive to outliers and prone to prototype shift. To resolve this critical weakness, we propose the Adaptive Prototype Construction Module (APCM). APCM dynamically assigns adaptive weights to each sample by jointly evaluating the intra-class compactness and inter-class repulsion of support set samples in the metric space. This mitigates interference from noisy samples and broadens decision boundaries between easily confused categories, establishing a robust metric baseline for scenarios with high inter-class similarity. For the inference stage, we introduce the Compactness-Aware Prototype Fusion (CPF) strategy. CPF quantitatively evaluates the distribution uncertainties of purely visual and semantically enhanced prototypes to achieve an adaptive dynamic fusion of both modalities, further improving the reliability of classification decisions under extreme data scarcity.

The contributions of this research center on a comprehensive multimodal framework that systematically resolves the severe challenges of background clutter, significant intra-class variance, and high inter-class similarity in remote sensing imagery, establishing a robust paradigm for classification tasks under extremely data-scarce conditions. Within this framework, the developed VGSR and DCMA modules introduce visually anchored semantic priors and advance cross-modal interactions to the feature extraction stage. They achieve explicit background noise suppression and overcome the representation bottlenecks associated with traditional late fusion methodologies. Concurrently, the APCM and CPF modules allocate dynamic weights based on dual intra-class and inter-class constraints in the metric space and perform uncertainty-based adaptive prototype fusion. Together, they prevent outlier-induced prototype distortion and significantly enhance classification robustness across highly similar categories. Comprehensive experimental evaluations on multiple mainstream few-shot remote sensing public datasets demonstrate that the proposed framework significantly outperforms existing state-of-the-art approaches, thoroughly verifying the effectiveness of the core modules in processing complex remote sensing scenes.
